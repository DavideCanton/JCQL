...
import jcql.builder.QueryBuilder;
import jcql.builder.RPNQueryBuilder;
import jcql.lex.ConcreteQueryLexicalAnalyzer;
import jcql.lex.QueryLexicalAnalyzer;
import jcql.lex.Token;
...
public class RecursiveDescentQueryParser implements QueryParser
{
     ...
     *
     * @param lex L'handler che restituisce i simboli.
     */
    ...
    private void addTerm() throws IOException
    {
        ...
        {
            TwoOperandsLeafOperator mulop =
                    lex.currentToken() == Token.MUL ?
                            new MulOperator() :
                            lex.currentToken() == Token.DIV
                                    ? new DivOperator()
                                    : new ModOperator();
            lex.accept(lex.currentToken());
            mulTerm();
    ...
    private void firstValue() throws IOException
    {
        ...
        {
            StringConstant s = new StringConstant(lex.getString());
            lex.accept(Token.ID);
            builder.buildLeaf(s);
     ...
     *
     * @param s La stringa di cui fare il parsing.
     * @return L'espressione corrispondente alla stringa.
     * @throws InvalidFieldException     Se il campo specificato non esiste.
     * @throws InvalidOperationException Se l'operazione specificata non è applicabile ai tipi dei campi.
     */
